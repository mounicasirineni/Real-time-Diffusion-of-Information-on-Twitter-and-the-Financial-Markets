...
-- step1
SELECT count(*), symbol FROM (SELECT DISTINCT(datestart), symbol FROM Tweets) 
AS t GROUP BY symbol HAVING count(*)>=30;
SELECT count(*) FROM (SELECT count(*), symbol FROM (SELECT DISTINCT(datestart), symbol FROM Tweets) 
AS t GROUP BY symbol HAVING count(*)>=30) as c;


-- step 2a
SELECT T.smblid, avg(T.twitterpermin) FROM Tweets T, EarnRelMatched E1 where T.datestart = E1.earnrelease_date  
and T.smblid = E1.ticker and T.timestart between E1.earnrelease_time 
and date_add(E1.earnrelease_time, interval 40 minute)
group by T.smblid, T.datestart;


-- step 2b
SELECT T.smblid, avg(T.twitterpermin) FROM Tweets T, EarnRelMatched E1 where T.datestart = E1.earnrelease_date  
and T.smblid = E1.ticker and T.timestart between E1.earnrelease_time 
and date_add(E1.earnrelease_time, interval 2 hour)
group by T.smblid, T.datestart;
...
SELECT T.smblid, avg(T.twitterpermin) FROM Tweets T, EarnRelMatched E1 where T.datestart = E1.earnrelease_date  
and T.smblid = E1.ticker and T.timestart between E1.earnrelease_time 
and date_add(E1.earnrelease_time, interval 24 hour)
group by T.smblid, T.datestart;
...
SELECT T.smblid, avg(T.twitterpermin) FROM Tweets T, EarnRelMatched E1 where T.datestart = E1.earnrelease_date  
and T.smblid = E1.ticker and T.timestart between E1.earnrelease_time 
and date_add(E1.earnrelease_time, interval 1 week)
group by T.smblid, T.datestart;


-- step 2e

select t.symbol, avg(t.twitterpermin) from tweets t, earnrelmatched e where e.ticker=t.smblid and 
t.timestart between e.earnrelease_time and date_add(e.earnrelease_time, interval 30 minute) and dayofweek(t.datestart)=dayofweek(e.earnrelease_date)
and t.datestart between 2012/05/10 and e.earnrelease_date-7 group by t.smblid, e.earnrelease_date;


-- step 3a
SELECT T.smblid, avg(T.volumeend) FROM Tweets T, EarnRelMatched E1 where T.datestart = E1.earnrelease_date  
and T.smblid = E1.ticker and T.timestart between E1.earnrelease_time 
and date_add(E1.earnrelease_time, interval 40 minute)
group by T.smblid, T.datestart;
...
-- step 3e
select t.symbol, avg(t.volumeend) from tweets t, earnrelmatched e where e.ticker=t.smblid and 
t.timestart between e.earnrelease_time and date_add(e.earnrelease_time, interval 30 minute) and dayofweek(t.datestart)=dayofweek(e.earnrelease_date)
and t.datestart between 2012/05/10 and e.earnrelease_date-7 group by t.smblid, e.earnrelease_date;
